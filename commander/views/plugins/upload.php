    ...
    <form method="POST" action="/plugins/upload" enctype="multipart/form-data">
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="name" placeholder="e.g. Elementor Pro" required 
                   value="<?= isset($plugin) ? htmlspecialchars($plugin['name']) : '' ?>"
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="slug" placeholder="elementor-pro" 
                   value="<?= isset($plugin) ? htmlspecialchars($plugin['slug']) : '' ?>"
                   <?= isset($plugin) ? 'readonly style="width: 100%; padding: 0.75rem; background-color: var(--color-bg-dark); border: 1px solid var(--color-bg-hover); border-radius: var(--radius-md); color: var(--color-text-muted); opacity: 0.7;"' : 'style="width: 100%; padding: 0.75rem; background-color: var(--color-bg-dark); border: 1px solid var(--color-bg-hover); border-radius: var(--radius-md); color: var(--color-text-main);"' ?>>
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="version" placeholder="1.0.0" required 
                   value="<?= isset($plugin) ? htmlspecialchars($plugin['version']) : '' ?>"
                   style="width: 100%; padding: 0.75rem; background-color: var(--color-bg-dark); border: 1px solid var(--color-bg-hover); border-radius: var(--radius-md); color: var(--color-text-main);">
        </div>
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <div style="border: 2px dashed var(--color-bg-hover); padding: 2rem; text-align: center; border-radius: var(--radius-lg); cursor: pointer;" onclick="document.getElementById('file-input').click()">
                <div style="margin-bottom: 0.5rem; color: var(--color-primary);"><?= isset($plugin) ? 'Click to Replace File' : 'Click to Select File' ?></div>
        ...
        <div style="display: flex; gap: 1rem;">
            <button type="submit" style="flex: 1; padding: 0.875rem; background-color: var(--color-primary); color: white; border: none; border-radius: var(--radius-md); font-weight: 600; cursor: pointer;"><?= isset($plugin) ? 'Update Plugin' : 'Upload to Secure Storage' ?></button>
